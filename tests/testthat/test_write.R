...
test_that("We can add userId information and validate", {
    contact <- as("Matthew Jones <jones@nceas.ucsb.edu>", "contact")
    creator <- c(as(contact, "creator"))
    creator[[1]]@userId <- new("userId")
    creator[[1]]@userId@userId <- "http://orcid.org/0000-0003-0077-4738"
    creator[[1]]@userId@directory <- "http://orcid.org"
    uid <- EML:::eml_id()
    eml_write(new("eml",
                  packageId = uid[["id"]],
                  system = uid[["system"]],
